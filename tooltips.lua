...
local function SkinTooltip(tt)
    ...
    local borderPath = (LSM and LSM:Fetch("border", ns.db.tooltipBorderTexture or "Blizzard Tooltip")) or "Interface\\Tooltips\\UI-Tooltip-Border"
...
local function SkinTooltipStatusBar()
    ...
    local LSM = LibStub and LibStub("LibSharedMedia-3.0", true)
    local texPath = (LSM and LSM:Fetch("statusbar", ns.db.tooltipBarTexture or "Minimalist")) or "Interface\\TargetingFrame\\UI-StatusBar"
...
local function GetUnitClassColor(unit)
    ...
    if UnitIsPlayer(unit) then
        ...
        if reaction then
            if reaction >= 5 then return "ff20ff20" 
            elseif reaction == 4 then return "ffffff00" 
            else return "ffff2020" 
            end
        end
    end
    return "ffffffff"
...
local function ProcessTooltipData(tooltip, data)
    ...
    if isPlayer then
        ...
        if guildName then
            ...
            if guildStr then
                guildStr:SetText("|cff" .. guildHex .. "<" .. guildName .. ">|r |cff" .. rankHex .. (guildRank or "") .. "|r")
            end
        end
    else
        local nameStr = _G["GameTooltipTextLeft1"]
