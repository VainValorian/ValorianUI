...
local function TriggerUfUpdate()
    if ns.Modules["UnitFrames"] and ns.Modules["UnitFrames"].UpdateAllLayouts then
        ...
        if ValUI_UFUnlocked then
            for _, f in ipairs({ValorianUI_Player, ValorianUI_Target, ValorianUI_ToT, ValorianUI_Focus, ValorianUI_Party1, ValorianUI_Raid1}) do
...
local function BuildIconPositioners(canvas, title, prefix, flags, startY)
    ...
    if flags.aura then
        ...
        UI.AddDropdownList(canvas, "ValUI_"..prefix.."AuraDD", "Aura Anchor", "uf"..prefix.."AuraAnchor", anchorOptions, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."AuraSize", "Aura Size", "uf"..prefix.."AuraSize", 12, 40, 1, 300, y-15, TriggerUfUpdate)
        y = y - 60
        UI.AddSlider(canvas, "ValUI_"..prefix.."AuraX", "Aura X Offset", "uf"..prefix.."AuraX", -150, 150, 1, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."AuraY", "Aura Y Offset", "uf"..prefix.."AuraY", -150, 150, 1, 300, y, TriggerUfUpdate)
        y = y - 50
    end

    if flags.class then
        y = UI.AddSubHeader(canvas, title .. " Class Icon", y)
        UI.AddDropdownList(canvas, "ValUI_"..prefix.."ClassDD", "Class Anchor", "uf"..prefix.."ClassAnchor", anchorOptions, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."ClassSize", "Class Size", "uf"..prefix.."ClassSize", 12, 40, 1, 300, y-15, TriggerUfUpdate)
        y = y - 60
        UI.AddSlider(canvas, "ValUI_"..prefix.."ClassX", "Class X Offset", "uf"..prefix.."ClassX", -150, 150, 1, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."ClassY", "Class Y Offset", "uf"..prefix.."ClassY", -150, 150, 1, 300, y, TriggerUfUpdate)
        y = y - 50
    end

    if flags.status then
        y = UI.AddSubHeader(canvas, title .. " Status Icon (Combat/Resting)", y)
        UI.AddDropdownList(canvas, "ValUI_"..prefix.."StatusDD", "Status Anchor", "uf"..prefix.."StatusAnchor", anchorOptions, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."StatusSize", "Status Size", "uf"..prefix.."StatusSize", 12, 40, 1, 300, y-15, TriggerUfUpdate)
        y = y - 60
        UI.AddSlider(canvas, "ValUI_"..prefix.."StatusX", "Status X Offset", "uf"..prefix.."StatusX", -150, 150, 1, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."StatusY", "Status Y Offset", "uf"..prefix.."StatusY", -150, 150, 1, 300, y, TriggerUfUpdate)
        y = y - 50
    end

    if flags.group then
        y = UI.AddSubHeader(canvas, title .. " Group Icons (Leader/Role/Loot)", y)
        UI.AddDropdownList(canvas, "ValUI_"..prefix.."GroupDD", "Group Anchor", "uf"..prefix.."GroupAnchor", anchorOptions, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."GroupSize", "Group Size", "uf"..prefix.."GroupSize", 12, 40, 1, 300, y-15, TriggerUfUpdate)
        y = y - 60
        UI.AddSlider(canvas, "ValUI_"..prefix.."GroupX", "Group X Offset", "uf"..prefix.."GroupX", -150, 150, 1, 20, y, TriggerUfUpdate)
        UI.AddSlider(canvas, "ValUI_"..prefix.."GroupY", "Group Y Offset", "uf"..prefix.."GroupY", -150, 150, 1, 300, y, TriggerUfUpdate)
        y = y - 50
    end
...
function Options:BuildRootPage(mainPanel)
    ...
    desc:SetText("Welcome to the Valorian UI Configuration panel. Use the sub-categories on the left to customize your dark-fantasy interface.")
    
    y = y - 40
    local charKey = ns.charKey
    local profileCb = UI.AddCheckbox(mainPanel, "|cffFFD100Enable Private Character Profile|r", "useProfile_"..charKey, false, 20, y, nil, function(isChecked)
    ...
    unlockArtBtn:SetScript("OnClick", function(self)
        ValUI_FramesUnlocked = not ValUI_FramesUnlocked
        self:SetText(ValUI_FramesUnlocked and "Lock Art Frames" or "Unlock Art Frames")
            ...
            if f then
                if ValUI_FramesUnlocked then
                    if not ((name == "ValorianUIHealthOrb") and InCombatLockdown()) then
                        ...
                        if not f.originalStrata then f.originalStrata = f:GetFrameStrata(); f.originalLevel = f:GetFrameLevel() end
                    ...
                    if name ~= "ValorianUIHealthOrb" then 
                        f:EnableMouse(false) 
                        if f.originalStrata then f:SetFrameStrata(f.originalStrata); f:SetFrameLevel(f.originalLevel) end
                    end
    ...
    unlockUFBtn:SetScript("OnClick", function(self)
        if InCombatLockdown() then print("|cffFF0000ValorianUI:|r Cannot toggle Test Mode in combat."); return end
        ValUI_UFUnlocked = not ValUI_UFUnlocked
        self:SetText(ValUI_UFUnlocked and "Lock Unit Frames" or "Unlock Unit Frames (Test Mode)")
...
function Options:BuildGeneralSettings(parentCategory)
    ...
    UI.AddCheckbox(canvas, "Main Action Bar Art", "showActionBars", true, 20, y, {"ValorianUIBar1", "ValorianUIBar2", "ValorianUIBar1Mask", "ValorianUIBar2Mask"})
    UI.AddCheckbox(canvas, "Extra Action Bar Art", "showExtraActionBars", true, 300, y, {"ValorianUIArt_MultiBarBottomRight", "ValorianUIArt_MultiBarRight", "ValorianUIArt_MultiBarLeft", "ValorianUIArt_MultiBar5", "ValorianUIArt_MultiBar6", "ValorianUIArt_MultiBar7"})
    y = y - 30
    UI.AddCheckbox(canvas, "Chat & Damage Backdrops", "showChatDamage", true, 20, y, {"ValorianUIChatArt", "ValorianUIDamageArt"})
    ...
    UI.AddCheckbox(canvas, "Enable Minimap Drawer (Reload)", "showMinimapDrawer", true, 300, y, nil, function() print("|cff888888ValorianUI:|r Please type /reload to apply.") end)
    y = y - 40

    y = UI.AddSubHeader(canvas, "Art Scaling", y)
    local function ScaleCb(f) return function(v) if not ns.db.frames[f] then ns.db.frames[f]={} end ns.db.frames[f].scale=v if _G[f] then _G[f]:SetScale(v) end end end
    
    UI.AddSlider(canvas, "ValUISliderChat", "Chat Art Scale", "scale_chat", 0.5, 2.5, 0.05, 20, y, ScaleCb("ValorianUIChatArt"))
    UI.AddSlider(canvas, "ValUISliderDmg", "Damage Art Scale", "scale_dmg", 0.5, 2.5, 0.05, 300, y, ScaleCb("ValorianUIDamageArt"))
    y = y - 50
    UI.AddSlider(canvas, "ValUISliderHealth", "Health Orb Scale", "scale_health", 0.5, 2.5, 0.05, 20, y, ScaleCb("ValorianUIHealthOrb"))
    UI.AddSlider(canvas, "ValUISliderPower", "Power Orb Scale", "scale_power", 0.5, 2.5, 0.05, 300, y, ScaleCb("ValorianUIPowerOrb"))
    y = y - 50
    UI.AddSlider(canvas, "ValUISliderSucc", "Succubus Art Scale", "scale_succ", 0.5, 2.5, 0.05, 20, y, ScaleCb("ValorianUISuccubusFrame"))
    UI.AddSlider(canvas, "ValUISliderInfo", "Info Bar Scale", "scale_info", 0.5, 2.5, 0.05, 300, y, ScaleCb("ValorianUI_InfoBar"))
    y = y - 50
    
    y = UI.AddSubHeader(canvas, "Global Fonts", y)
    UI.AddDropdownLSM(canvas, "ValUIFontDropdown", "Global UI Font", "fontChoice", "font", 20, y, function(v) if LSM and ValorianUI_ApplyGlobalFonts then ValorianUI_ApplyGlobalFonts(LSM:Fetch("font", v)) end end)
    y = y - 50
    UI.AddCheckbox(canvas, "Use Class Color for Tracker", "trackerClassColor", false, 20, y, nil, function() if ObjectiveTrackerFrame then ObjectiveTrackerFrame:Update() end end)
    UI.AddColorPicker(canvas, "Custom Tracker Color", "trackerColor", 0.8, 0.2, 0.2, 300, y, function() if ObjectiveTrackerFrame then ObjectiveTrackerFrame:Update() end end)
...
function Options:BuildUnitFramesMaster(parentCategory)
    ...
    UI.AddDropdownLSM(canvas, "ValUIHlthTexDD", "Global Health Bar Texture", "ufHealthTexture", "statusbar", 20, y, Notify)
...
function Options:BuildUFPlayerSettings(ufCategory)
    ...
    UI.AddSlider(canvas, "ValUI_PlayerBorderInset", "Border Offset", "ufPlayerBorderOffset", 0, 10, 1, 300, y, TriggerUfUpdate)
    y = y - 50
    UI.AddDropdownLSM(canvas, "ValUI_PlayerBorderDD", "Border Texture", "ufPlayerBorder", "border", 20, y, TriggerUfUpdate)
    y = y - 60
    ...
    UI.AddDropdownLSM(canvas, "ValUINatCastTexDD", "Castbar Texture", "nativeCastbarTexture", "statusbar", 20, y, TriggerUfUpdate)
    y = y - 60
...
function Options:BuildUFTargetSettings(ufCategory)
    ...
    UI.AddSlider(canvas, "ValUI_TarBorderInset", "Border Offset", "ufTargetBorderOffset", 0, 10, 1, 300, y, TriggerUfUpdate)
    y = y - 50
    ...
    UI.AddDropdownLSM(canvas, "ValUITarCastTexDD", "Castbar Texture", "ufCastbarTexture", "statusbar", 20, y, TriggerUfUpdate)
    y = y - 60
...
function Options:BuildUFGroupSettings(ufCategory)
    ...
    y = y - 50
    UI.AddSlider(canvas, "ValUI_PartyBorderInset", "Border Offset", "ufPartyBorderOffset", 0, 10, 1, 20, y, TriggerUfUpdate)
...
function Options:BuildUFEncounterSettings(ufCategory)
    ...
    UI.AddSlider(canvas, "ValUI_EncBorderInset", "Border Offset", "ufEncounterBorderOffset", 0, 10, 1, 300, y, TriggerUfUpdate)
    y = y - 50
    UI.AddDropdownLSM(canvas, "ValUI_EncBorderDD", "Border Texture", "ufEncounterBorder", "border", 20, y, TriggerUfUpdate)
    y = y - 60

    y = BuildIconPositioners(canvas, "Encounter", "Encounter", {aura=true, class=true, status=false, group=false}, y)
...
function Options:BuildDamageMeterSettings(parentCategory)
    ...
    canvas.isDirty = true
    canvas:HookScript("OnShow", function(self) self.isDirty = true end)
    canvas:HookScript("OnUpdate", function(self) if self.isDirty then self:Layout(); self.isDirty = false end end)
    ...
    UI.AddDropdownLSM(canvas, "ValUIDMBorderDropdown", "Bar Border Texture", "meterBorderTexture", "border", 300, y, RequestUpdate)
    y = y - 50
    UI.AddSlider(canvas, "ValUIDMOffsetSlider", "Border Spacing Offset", "meterBorderOffset", -5, 15, 1, 20, y, RequestUpdate)
    y = y - 50
    ...
    y = y - 30
    UI.AddDropdownLSM(canvas, "ValUIDMIconBorderDropdown", "Icon Border Texture", "meterIconBorderTexture", "border", 20, y, RequestUpdate)
    UI.AddSlider(canvas, "ValUIDMIconOffsetSlider", "Icon Border Offset", "meterIconBorderOffset", -5, 15, 1, 300, y-15, RequestUpdate)
...
function Options:BuildStatusBarsSettings(parentCategory)
    ...
    function canvas:Layout()
        ...
        if self.previewColorKey == "sbExpColor" then previewText:SetText("Experience 65%")
        elseif self.previewColorKey == "sbRepColor" then previewText:SetText("Reputation 65%")
        elseif self.previewColorKey == "sbHonorColor" then previewText:SetText("Honor 65%")
        end
    end
    ...
    UI.AddDropdownLSM(canvas, "ValUISBBorderDropdown", "Bar Border Texture", "statusBorderTexture", "border", 300, y, RequestUpdate)
    y = y - 50
    ...
    UI.AddColorPicker(canvas, "Experience", "sbExpColor", 0.5, 0.2, 0.8, 20, y, function() canvas.previewColorKey = "sbExpColor"; RequestUpdate() end)
    UI.AddColorPicker(canvas, "Reputation", "sbRepColor", 0.2, 0.6, 0.8, 140, y, function() canvas.previewColorKey = "sbRepColor"; RequestUpdate() end)
    UI.AddColorPicker(canvas, "Honor", "sbHonorColor", 0.8, 0.2, 0.2, 260, y, function() canvas.previewColorKey = "sbHonorColor"; RequestUpdate() end)
...
function Options:BuildChatSettings(parentCategory)
    ...
    UI.AddCheckbox(canvas, "Enable Valorian Chat Engine (Reload)", "enableChatEngine", true, 20, y, nil, function() print("|cff888888ValorianUI:|r Please type /reload to apply.") end)
    UI.AddCheckbox(canvas, "Lock Chat Frame (Hides Resize Handle)", "chatLocked", false, 300, y, nil, function() if ns.ChatEngine then ns.ChatEngine:UpdateSettings() end end)
    y = y - 30
    UI.AddCheckbox(canvas, "Auto-Fade Tabs (Show on Mouseover)", "chatAutoFadeTabs", false, 20, y)
    UI.AddCheckbox(canvas, "Auto-Fade EditBox (Hide when not typing)", "chatAutoFadeEditBox", false, 300, y, nil, function() if ns.ChatEngine then ns.ChatEngine.UpdateEditBoxFade() end end)
    y = y - 30
    ...
    UI.AddSlider(canvas, "ValUI_ChatFontSlider", "Font Size", "chatFontSize", 10, 24, 1, 20, y, function() if ns.ChatEngine then ns.ChatEngine:UpdateSettings() end end)
    UI.AddSlider(canvas, "ValUI_ChatAlphaSlider", "Background Opacity", "chatBgAlpha", 0, 1, 0.05, 300, y, function() if ns.ChatEngine then ns.ChatEngine:UpdateSettings() end end)
...
function Options:BuildTooltipSettings(parentCategory)
    ...
    UI.AddCheckbox(canvas, "Enable Tooltip Engine (Reload)", "enableTooltips", true, 20, y, nil, function() print("|cff888888ValorianUI:|r Please type /reload to apply.") end)
    ...
    UI.AddSlider(canvas, "ValUITooltipBodySize", "Tooltip Scale (Size)", "tooltipBodySize", 10, 24, 1, 300, y, function() if ns.Modules["Tooltips"] and ns.Modules["Tooltips"].UpdateFonts then ns.Modules["Tooltips"]:UpdateFonts() end end)
    y = y - 60
